...
NTSTATUS:main() {
    ...
    // Validate that the mapped window looks like an SMC. Do not probe FNum with
    // a hard-coded length here: on T2 firmware its reported type/length may vary
    // by model. User mode queries the key type first and then reads the exact
    // length advertised by the controller.
    new status = 0;
    new NTSTATUS:st = virtual_read_byte(g_smc_va + SMC_KEY_STATUS, status);
    if (!NT_SUCCESS(st) || status == 0xFF) {
        io_space_unmap(g_smc_va, T2_SMC_SIZE);
        g_smc_va = NULL;
        return STATUS_NOT_SUPPORTED;
    }

    debug_print("AppleT2Smc: MMIO mapped, initial status=%x", status);
    return STATUS_SUCCESS;
}
