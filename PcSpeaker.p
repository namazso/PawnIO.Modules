...
//
// This module is intended only to generate tones through the legacy system
// speaker and to check whether the system-speaker-related hardware interface is
// present and responsive. It uses system timer channel 2, which is the channel
// associated with the system speaker; it does not access the timer channels
// used for Windows timekeeping.
//
// The hardware check covers system control port B and the system timer channel
// 2 feedback path. It cannot determine whether a physical speaker cone is
// connected or whether a generated tone is audible.
//
...
//
// The reason behind creating this module is HalMakeBeep is a ring0 function and cannot be called from user mode and if called from kernel mode, no sound is produced in modern systems.
//
// Also, there's a driver called Beep.sys that can be used to make a beep, but it uses ordinary speaker instead of the system speaker
// and there was a replacement called BeepXP64 but it has driver signature issues (the signature that can't be verified) and its download link is dead that only "404 - page not found" is shown when you click on the original download link. 
// So, this module is created to make a beep sound using the system speaker.
...
// Check whether a frequency is within the valid range for the system speaker.
bool:is_valid_frequency(frequency) {
    return frequency >= SPEAKER_MIN_HZ && frequency <= SPEAKER_MAX_HZ;
}

// Check whether the speaker gate and data bits are both enabled (stuck in the on state).
bool:is_speaker_enabled() {
    return (io_in_byte(SYSTEM_CONTROL_PORT_B) &
        SPEAKER_GATE_AND_DATA_ENABLE) == SPEAKER_GATE_AND_DATA_ENABLE;
}

// Check the state of the speaker control port to verify responsiveness of system speaker-related hardware.
...
// Check the system timer's responsiveness by counting the number of output transitions on channel 2 while the speaker data path is disabled. The caller can use the transition count to determine whether the system timer is functioning correctly.
...
// Check the legacy system speaker interface by verifying the speaker control port and system timer channel 2 feedback path. This cannot determine whether a physical speaker cone is connected or whether a generated tone is audible.
...
// Probe the system speaker by emitting a very short 9 kHz tone.
...
// Wake up the legacy system speaker interface by resetting the system timer channel 2 and toggling the speaker gate. This is a workaround for some systems that fails to beep and able to make beep after sleep and wake up and the issue starts again when rebooted.
...
// Stop any tone that may be playing through the legacy system speaker. This is a no-op if the speaker is already silent.
speaker_stop() {
...
// Start a continuous tone through the legacy system speaker at the specified frequency. The caller must call speaker_stop when the tone is no longer needed.
